...
type RankingRow = { playerId: string; playerName: string; value: number };

export async function GET(request: NextRequest) {
  const session = await getServerSession(authOptions);
  const currentPlayerId = session?.user?.playerProfileId;
  if (!currentPlayerId) return NextResponse.json({ error: "Authentication required" }, { status: 401 });
  ...
  if (!exerciseId || !exercise) return NextResponse.json({ error: "Unknown exercise" }, { status: 400 });
  ...
  if (scope === "club" && !clubId) {
    return NextResponse.json({ exerciseId, exerciseName: exercise.name, scope, valueLabel: getRankingMetric(exercise)?.label ?? "Score", direction: getDirection(exercise), rows: [] });
  }
  ...
  const results = await prisma.trainingResult.findMany({
    where: { exerciseId, ...(scope === "club" && clubId ? { clubId } : {}) },
    select: {
      playerId: true,
      metrics: true,
      player: { select: { displayName: true, username: true, userId: true } },
    },
  });
  ...
  for (const result of results) {
    ...
    const publicName = isLegacyProfileForCurrentPlayer
      ? currentProfile?.username ?? "Anonym spiller"
      : result.player.username ?? "Anonym spiller";

    if (isLegacyProfileForCurrentPlayer) legacyPlayerIds.add(result.playerId);
    ...
    if (isBetter) bestByPlayer.set(effectivePlayerId, { playerId: effectivePlayerId, playerName: publicName, value });
  }

  if (legacyPlayerIds.size > 0) {
    await prisma.trainingResult.updateMany({ where: { playerId: { in: [...legacyPlayerIds] } }, data: { playerId: currentPlayerId } });
  }

  const rows = [...bestByPlayer.values()]
    .sort((a, b) => (direction === "higher" ? b.value - a.value : a.value - b.value))
    .slice(0, 100)
    .map((row, index) => ({ place: index + 1, playerName: row.playerName, value: row.value, isCurrentPlayer: row.playerId === currentPlayerId }));

  return NextResponse.json({ exerciseId, exerciseName: exercise.name, scope, valueLabel: rankingMetric?.label ?? "Score", direction, rows });
}

function getRankingMetric(exercise: NonNullable<ReturnType<typeof getTrainingExercise>>) {
  return exercise.metrics.find((metric) => metric.key === "score") ?? exercise.metrics.find((metric) => metric.personalBest);
}
