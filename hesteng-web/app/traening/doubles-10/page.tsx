...
export default function Doubles10Page() {
    ...
    return (
      <main className="min-h-screen bg-gray-950 text-white">
        ...
        <section className="mx-auto flex min-h-[70vh] max-w-3xl flex-col justify-center px-5 py-12 text-center sm:px-8">
          <div className="text-xs font-black uppercase tracking-[0.22em] text-orange-400">Doubles 10</div>
  ...
  async function handleComplete({
    ...
    const result: TrainingResult = {
      id: `training-${DOUBLES_10_EXERCISE_ID}-${currentPlayerId}-${Date.now()}`,
      clubId: trainingClubId,
      playerId: currentPlayerId,
      exerciseId: DOUBLES_10_EXERCISE_ID,
    ...
    try {
      ...
      console.error("Failed to save Doubles 10 result", error);
      ...
      <section className="mx-auto max-w-4xl px-4 py-6 sm:px-6 sm:py-10">
        <div className="mb-5 flex items-center justify-between gap-3">
          <div>
            ...
            <h1 className="mt-1 text-3xl font-black sm:text-4xl">Doubles 10</h1>
          </div>
          <Link
            href="/traening"
        ...
        {savedResult ? (
          <section className="rounded-3xl border border-emerald-400/20 bg-emerald-400/10 p-5 text-center sm:p-7">
            ...
            <div className="mt-3 text-5xl font-black text-white">{savedResult.metrics.hits ?? 0} / 10</div>
            <div className="mt-1 text-sm text-gray-300">Doubler ramt</div>
            <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3">
              <ResultStat label="Double %" value={formatPercent(savedResult.metrics.doublePercent)} />
              <ResultStat label="Pile brugt" value={savedResult.metrics.totalDartsThrown} />
              <ResultStat label="Snit ved hit" value={formatAverage(savedResult.metrics.averageDartsOnHit)} />
            </div>
          ...
          <>
            <Doubles10Training key={sessionNumber} onComplete={handleComplete} />
