...
import ScoringTargetsTraining from "@/components/training/ScoringTargetsTraining";
import { useOptionalCurrentUser } from "@/context/CurrentUserContext";
import { SCORING_TARGETS_EXERCISE_ID } from "@/data/trainingExercises";
import { saveTrainingResultToSharedStore } from "@/lib/trainingResultStore";
import type { TrainingResult } from "@/lib/trainingTypes";

export default function ScoringTargetsPage() {
  ...
  async function handleComplete({ metrics, details }: { metrics: Record<string, number>; details: Record<string, unknown> }) {
    const result: TrainingResult = {
      id: `training-${SCORING_TARGETS_EXERCISE_ID}-${currentPlayerId}-${Date.now()}`,
      clubId: trainingClubId,
      playerId: currentPlayerId,
      exerciseId: SCORING_TARGETS_EXERCISE_ID,
    ...
    try {
      ...
      console.error("Failed to save Scoring Targets result", error);
      ...
      <section className="mx-auto max-w-4xl px-4 py-6 sm:px-6 sm:py-10">
        <div className="mb-5 flex items-center justify-between gap-3">
          <div>
            ...
            <h1 className="mt-1 text-3xl font-black sm:text-4xl">Scoring Targets</h1>
          </div>
        ...
        {savedResult ? (
          <section className="rounded-3xl border border-emerald-400/20 bg-emerald-400/10 p-5 text-center sm:p-7">
            ...
            <div className="mt-3 text-5xl font-black text-white">{savedResult.metrics.score ?? 0}</div>
            <div className="mt-1 text-sm text-gray-300">Point</div>
            <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
              <ResultStat label="Træf %" value={formatPercent(savedResult.metrics.hitPercent)} />
              <ResultStat label="T20 hits" value={savedResult.metrics.t20Hits} />
              <ResultStat label="T19 hits" value={savedResult.metrics.t19Hits} />
              <ResultStat label="Bull hits" value={savedResult.metrics.bullHits} />
            </div>
            <div className="mt-6 flex flex-col justify-center gap-3 sm:flex-row">
              <Link href="/player/historik" className="rounded-2xl bg-white px-5 py-3 font-bold text-gray-950 transition hover:bg-gray-200">Se historik</Link>
              <button type="button" onClick={() => { setSavedResult(null); setSaveError(null); setSessionNumber((value) => value + 1); }} className="rounded-2xl border border-white/20 px-5 py-3 font-bold text-white transition hover:bg-white/10">Træn igen</button>
            </div>
          </section>
        ) : (
          <>
            <ScoringTargetsTraining key={sessionNumber} onComplete={handleComplete} />
...
function ResultStat({ label, value }: { label: string; value: unknown }) {
  return (
    <div className="rounded-2xl bg-black/20 p-3">
      ...
      <div className="mt-1 text-xl font-black text-white">{typeof value === "number" || typeof value === "string" ? value : "-"}</div>
    </div>
  );
}
