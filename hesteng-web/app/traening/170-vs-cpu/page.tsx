...
import Checkout170VsCpuTraining from "@/components/training/Checkout170VsCpuTraining";
import { useOptionalCurrentUser } from "@/context/CurrentUserContext";
import { CHECKOUT_170_VS_CPU_EXERCISE_ID } from "@/data/trainingExercises";
import { saveTrainingResultToSharedStore } from "@/lib/trainingResultStore";
import type { TrainingResult } from "@/lib/trainingTypes";

export default function Checkout170VsCpuPage() {
    ...
    return (
      <main className="min-h-screen bg-gray-950 text-white">
        ...
        <section className="mx-auto flex min-h-[70vh] max-w-3xl flex-col justify-center px-5 py-12 text-center sm:px-8">
          <div className="text-xs font-black uppercase tracking-[0.22em] text-orange-400">170 vs CPU</div>
  ...
  async function handleComplete({
    ...
    const result: TrainingResult = {
      id: `training-${CHECKOUT_170_VS_CPU_EXERCISE_ID}-${currentPlayerId}-${Date.now()}`,
      clubId: trainingClubId,
      playerId: currentPlayerId,
      exerciseId: CHECKOUT_170_VS_CPU_EXERCISE_ID,
    ...
    try {
      ...
      console.error("Failed to save 170 vs CPU result", error);
      ...
      <section className="mx-auto max-w-4xl px-4 py-6 sm:px-6 sm:py-10">
        <div className="mb-5 flex items-center justify-between gap-3">
          <div>
            ...
            <h1 className="mt-1 text-3xl font-black sm:text-4xl">170 vs CPU</h1>
          </div>
          <Link
            href="/traening"
        ...
        {savedResult ? (
          <section className="rounded-3xl border border-emerald-400/20 bg-emerald-400/10 p-5 text-center sm:p-7">
            <div className="text-sm font-bold uppercase tracking-[0.2em] text-emerald-300">Kamp afsluttet</div>
            <div className="mt-3 text-5xl font-black text-white">
              {savedResult.metrics.playerLegs ?? 0} - {savedResult.metrics.cpuLegs ?? 0}
            </div>
            <div className="mt-1 text-sm text-gray-300">
              {savedResult.metrics.won === 1 ? "Sejr" : "Nederlag"} mod CPU {savedResult.metrics.cpuLevel ?? "-"}
            </div>
            <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3">
              <ResultStat label="Dine legs" value={savedResult.metrics.playerLegs} />
              <ResultStat label="CPU legs" value={savedResult.metrics.cpuLegs} />
          ...
          <>
            <Checkout170VsCpuTraining key={sessionNumber} onComplete={handleComplete} />
