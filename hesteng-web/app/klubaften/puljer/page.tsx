...
import { createClubNightPools, type PoolMode, type PoolSizeProfile } from "@/lib/thuPoolEngine";
...
export default function PuljerPage() {
  ...
  if (displayedSelectedPlayers.length < minimumPlayers) {
    return (
      <main className="min-h-screen bg-gray-950 text-white">
        ...
        <section className="mx-auto max-w-5xl p-10">
          ...
          <div className="rounded-2xl border border-gray-800 bg-gray-900 p-8 text-center text-gray-400">
            Vælg mindst <strong>{minimumPlayers} spillere</strong> for at oprette puljer.
          </div>
        </section>
      </main>
    );
    ...
    <main className="min-h-screen bg-gray-950 text-white">
      ...
      <section className="mx-auto max-w-6xl p-10">
        ...
        <div className="mb-8">
          ...
          {isTjoerring ? <p className="mt-1 text-sm text-gray-500">Tjørring: 3-4 spillere pr. pulje, med 4 som førstevalg.</p> : null}
        </div>
        ...
        <div className="mb-8 rounded-2xl border border-gray-800 bg-gray-900 p-6">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            ...
            <button type="button" onClick={() => generatePools("elo")} className={`rounded-xl border px-5 py-4 text-left transition ${poolMode === "elo" && displayedPools.length > 0 ? "border-orange-500 bg-orange-500/10" : "border-gray-700 bg-gray-800 hover:border-gray-600"}`}>
        ...
        {displayedPools.length > 0 ? (
          <>
            <div className="mb-6 flex items-end justify-between gap-4">
              <p className="text-gray-400">{displayedSelectedPlayers.length} spillere fordelt i {displayedPools.length} puljer.</p>
              <div className="rounded-xl bg-green-500/10 px-4 py-2 text-sm font-semibold text-green-400">✓ Puljer oprettet</div>
            </div>
              ...
              {displayedPools.map((pool: Pool) => (
                <div key={pool.name} className="rounded-2xl border border-gray-800 bg-gray-900 p-6">
                  <div className="mb-4 flex items-center justify-between">
                    ...
                    <span className="rounded-full bg-gray-800 px-3 py-1 text-sm text-gray-300">{pool.players.length} spillere</span>
                  </div>
                      ...
                      return (
                        <div key={player} className="flex items-center justify-between gap-3 rounded-lg bg-gray-800 px-4 py-3">
                          <div className="min-w-0">
                            ...
                            {playerInfo.requiresAccessibleBoard ? <span className="mt-1 inline-flex rounded-full border border-orange-500/50 bg-orange-500/10 px-2 py-0.5 text-[0.65rem] font-black uppercase tracking-wide text-orange-300">Handicapbane</span> : null}
                          </div>
            ...
            <Link href={clubNightId ? `/klubaften/${clubNightId}/kampe` : "/klubaften/kampe"} className="mt-8 block w-full rounded-xl bg-orange-500 py-3 text-center text-lg font-semibold hover:bg-orange-600">
              Generér kampe + tildel baner
            </Link>
          </>
        ) : (
          <div className="rounded-2xl border border-dashed border-gray-700 p-8 text-center text-gray-500">Vælg en puljetype ovenfor for at oprette puljerne.</div>
        )}
      </section>
    </main>
  );
