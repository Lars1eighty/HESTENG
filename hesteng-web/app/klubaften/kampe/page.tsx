...
import { publishGuestClubNight } from "@/lib/publicClubNightClient";
...
export default function KampePage() {
  ...
  useEffect(() => {
    if (!clubNightId || !currentClubNight?.clubId || currentClubNight.status !== "active" || matches.length === 0) return;
    void publishGuestClubNight({
      clubNightId,
      clubId: currentClubNight.clubId,
      status: currentClubNight.status,
      clubNight: { ...currentClubNight, matches },
      completedMatches: [],
    }).catch(() => undefined);
  }, [clubNightId, currentClubNight?.clubId, currentClubNight?.status, matches]);
