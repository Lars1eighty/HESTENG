...
export default function KampePage() {
  ...
  return <main className="min-h-screen bg-gray-950 text-white"><Header /><section className="mx-auto max-w-7xl p-10"><BackButton />
    <div className="mb-8 flex items-end justify-between gap-4"><div><h1 className="text-4xl font-bold">🎯 Kampe + baner</h1><p className="mt-2 text-gray-400">Færdige {finished} · I gang {live} · Mangler {pending} · {activeBestOf === 1 ? "1 leg" : `Best of ${activeBestOf}`}</p></div>{clubNightId ? <Link href={`/klubaften/${clubNightId}?tv=1`} target="_blank" className="rounded-xl bg-orange-500 px-5 py-3 text-sm font-black text-black hover:bg-orange-400">📺 Live TV</Link> : null}</div>
