...
export default function KampePage() {
  ...
  useEffect(() => {
    if (pools.length > 0 && matches.length === 0 && clubNightId && currentClubNight?.status === "active") {
      setMatches(createClubNightMatches(pools, currentClubNight.boardCount, clubNightId, currentClubId, 5, currentClubNight.handicapBoards));
    }
  }, [clubNightId, currentClubId, currentClubNight, pools, matches.length, setMatches]);
  ...
  const boardCount = currentClubNight?.boardCount ?? Math.max(...matches.map((m) => m.board), 1);
  const boards = Array.from({ length: boardCount }, (_, index) => index + 1);
  ...
  return <main className="min-h-screen bg-gray-950 text-white"><Header /><section className="mx-auto max-w-7xl p-10"><BackButton />
    <div className="mb-8 flex items-end justify-between gap-4"><div><h1 className="text-4xl font-bold">🎯 Kampe + baner</h1><p className="mt-2 text-gray-400">Færdige {finished} · I gang {live} · Mangler {pending}</p></div><div className="flex gap-3"><Link href={clubNightId ? `/klubaften/${clubNightId}/puljer` : "/klubaften/puljer"} className="rounded-xl border border-gray-700 px-4 py-2 text-sm hover:bg-gray-800">Se puljer</Link><Link href={clubNightId ? `/klubaften/${clubNightId}/live` : "/klubaften/live"} className="rounded-xl bg-orange-500 px-4 py-2 text-sm font-semibold hover:bg-orange-600">🔴 Live scoring</Link></div></div>
