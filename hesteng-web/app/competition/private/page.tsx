...
export default function PrivateCompetitionPage() {
  ...
  async function publishGuestAccess(matches: GeneratedMatch[], pools: GeneratedPool[]) {
    ...
    const response = await fetch("/api/competition-public", {
      ...
      body: JSON.stringify({ competitionId, competition: { name: name.trim(), startingScore, pools, matches: matches.map((match) => ({ ...match, pool: match.round, startingScore, bestOfLegs })) }, completedMatches: [] }),
    });
    if (!response.ok) {
