...
test("real one-leg club match finishes at checkout and saves the result", async ({ page }) => {
  ...
  page.on("console", (message) => { if (message.type() === "error") browserErrors.push(`console: ${message.text()}`); if (message.text().includes("HESTENG_E2E_SCORER_STATE")) console.log("BROWSER_SCORER_STATE:", message.text()); });
  const clubNight = {
    id: "e2e-night",
