...
type ClubNightLike = { matches?: unknown };
...
function hasValidOutcome(completed: CompletedMatchLike, allowed: MatchLike) {
  ...
  const expectedWinner = (completed.score1 as number) > (completed.score2 as number) ? allowed.player1 : allowed.player2;
...
export function validateGuestCompletedMatches(clubNight: unknown, completedMatches: unknown[]) {
  const allowedMatches = new Map(getMatches(clubNight).filter((match) => typeof match.id === "string").map((match) => [match.id as string, match]));
  ...
  for (const completedValue of completedMatches) {
    ...
    if (typeof allowed.player1Id === "string" && completed.player1Id !== allowed.player1Id) return false;
    if (typeof allowed.player2Id === "string" && completed.player2Id !== allowed.player2Id) return false;
