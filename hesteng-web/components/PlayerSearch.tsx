...
export default function PlayerSearch() {
  ...
  const exactPlayer = trimmedSearch
    ? players.find((player) => normalizeName(player.name) === normalizeName(trimmedSearch))
    : undefined;
  ...
  function selectExistingPlayer(player: string) {
    setDraftPlayers((current) => (
      current.some((name) => normalizeName(name) === normalizeName(player))
        ? current
        : [...current, player]
    ));
    setSearch("");
  ...
  function requestPlayerFromSearch() {
    if (!trimmedSearch) return;

    if (exactPlayer) {
      selectExistingPlayer(exactPlayer.name);
      return;
    }
  ...
  return (
    <div className="rounded-2xl border border-gray-800 bg-gray-900 p-8">
      ...
      <div className="mt-8">
        <h3 className="mb-4 text-lg font-semibold">Valgte spillere ({selectedPlayers.length})</h3>
        {selectedPlayers.length === 0 ? (
          <div className="rounded-xl border border-dashed border-gray-700 p-6 text-center text-gray-500">Ingen spillere valgt.</div>
        ) : (
          <div className="space-y-2">
            {selectedPlayers.map((player) => {
              const profile = playerByName.get(normalizeName(player));
              return (
                <div key={player} className="flex items-center justify-between rounded-lg bg-gray-800 px-4 py-3">
                  <div className="min-w-0">
                    <div className="truncate">{player}</div>
                    {profile?.requiresAccessibleBoard ? (
                      <span className="mt-1 inline-flex rounded-full border border-orange-500/50 bg-orange-500/10 px-2 py-0.5 text-[0.65rem] font-black uppercase tracking-wide text-orange-300">Handicapbane</span>
                    ) : null}
                  </div>
                </div>
              );
          ...
          <div className="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-3xl border border-gray-800 bg-gray-950 shadow-2xl shadow-black">
            <div className="border-b border-gray-800 p-4 sm:p-5">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <button type="button" onClick={() => setIsOpen(false)} className="rounded-full border border-gray-700 px-4 py-2 text-sm font-bold text-gray-300 hover:border-orange-400 hover:text-orange-200">Annuller</button>
              </div>
              ...
              <div className="mt-4 flex gap-2">
                ...
                />
                {trimmedSearch ? (
                  <button type="button" onClick={requestPlayerFromSearch} className="rounded-2xl bg-orange-500 px-4 py-3 text-sm font-black text-black hover:bg-orange-400">
                    {exactPlayer ? "Vælg" : "Opret"}
                  </button>
                ) : null}
              </div>

              {pendingNewPlayer ? (
                <div className="mt-3 rounded-2xl border border-orange-500/40 bg-orange-500/10 p-4">
                  <p className="font-bold text-white">
                    {pendingNewPlayer} findes ikke i {currentClub.name}. Opret som ny spiller?
                  </p>
                  <div className="mt-3 flex gap-2">
                    <button type="button" onClick={confirmNewPlayer} className="rounded-xl bg-orange-500 px-4 py-2 text-sm font-black text-black hover:bg-orange-400">Ja, opret</button>
                    <button type="button" onClick={() => setPendingNewPlayer(null)} className="rounded-xl border border-gray-700 px-4 py-2 text-sm font-bold text-gray-300 hover:border-gray-500">Nej</button>
                  </div>
                </div>
              ) : null}

              <div className="mt-3 flex flex-wrap gap-2">
                <button type="button" onClick={selectLiveActive} className="rounded-full border border-green-500/50 bg-green-500/10 px-4 py-2 text-xs font-black uppercase tracking-wide text-green-200 hover:bg-green-500/20">Vælg Live Aktiv</button>
                <button type="button" onClick={clearDraft} className="rounded-full border border-gray-700 px-4 py-2 text-xs font-black uppercase tracking-wide text-gray-300 hover:border-red-400 hover:text-red-200">Ryd valg</button>
              </div>
            </div>
                ...
                {filteredPlayers.map((player) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex min-h-14 items-center gap-3 rounded-2xl border px-4 py-3 text-left transition ${checked ? "border-orange-400 bg-orange-500/15 text-white" : "border-gray-800 bg-gray-900 text-gray-200 hover:border-gray-600"}`}
                    >
                      <input type="checkbox" checked={checked} readOnly tabIndex={-1} className="h-5 w-5 shrink-0 accent-orange-500" />
            ...
            <div className="border-t border-gray-800 bg-gray-950 p-4 sm:p-5">
              {saveError && <div className="mb-3 rounded-2xl border border-red-800 bg-red-500/10 px-4 py-3 text-sm font-semibold text-red-200">{saveError}</div>}
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="text-lg font-black text-white">{draftPlayers.length} spillere valgt</div>
                <button type="button" onClick={savePlayers} className="min-h-12 rounded-2xl bg-green-500 px-6 py-3 text-sm font-black uppercase tracking-wide text-black hover:bg-green-400">Gem spillere</button>
              </div>
            </div>
          </div>
        </div>
