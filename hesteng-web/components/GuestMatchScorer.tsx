...
function toStats(state: PlayerState, name: string, playerId?: string): GuestPlayerStats {
  const average = state.entries > 0 ? state.totalScored / state.entries : 0;
  return {
    playerId,
    name,
    legs: state.legs,
    ...
    checkoutAttempts: state.checkouts,
    checkoutPercent: state.checkouts > 0 ? 100 : 0,
...
export default function GuestMatchScorer({
  ...
  async function submitScore() {
    ...
    if (!bust) {
      ...
      if (value === 180) active.oneEighties += 1;
    }
    ...
    if (!bust && after === 0) {
      ...
      if (active.legs >= legsToWin) {
        ...
        const result: GuestMatchScorerResult = {
          ...
          timingSource: "hesteng-scorer",
          players: [toStats(next[0], player1, player1Id), toStats(next[1], player2, player2Id)],
        };
        try {
  ...
  return (
    <div className="rounded-2xl border border-orange-500/40 bg-gray-900 p-4">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <div className="text-xs font-black uppercase tracking-widest text-orange-400">Scoreboard</div>
          <div className="mt-1 text-sm font-semibold text-gray-400">Først til {legsToWin} leg{legsToWin === 1 ? "" : "s"}</div>
        </div>
        {onCancel && <button type="button" onClick={onCancel} disabled={saving} className="rounded-lg border border-gray-700 px-3 py-2 text-sm font-bold text-gray-300 disabled:opacity-50">Luk</button>}
      </div>

      <div className="grid grid-cols-2 gap-3">
        {[0, 1].map((index) => (
          <div key={names[index]} className={`rounded-xl border p-4 ${currentPlayer === index ? "border-orange-500 bg-orange-500/10" : "border-gray-800 bg-gray-950"}`}>
            <div className="truncate text-sm font-black">{names[index]}</div>
            <div className="mt-2 text-4xl font-black tabular-nums">{players[index].remaining}</div>
            <div className="mt-2 text-xs font-bold text-gray-500">Legs {players[index].legs}</div>
          </div>
        ))}
      </div>
      ...
      <div className="mt-3 flex gap-2">
        <input
          ...
          autoFocus
          placeholder="Score"
          className="min-w-0 flex-1 rounded-xl border border-gray-700 bg-gray-950 px-4 py-4 text-center text-2xl font-black outline-none focus:border-orange-500 disabled:opacity-50"
        />
        <button type="button" onClick={() => void submitScore()} disabled={disabled || saving} className="rounded-xl bg-orange-500 px-5 py-4 font-black text-black disabled:opacity-50">ENTER</button>
      </div>

      <div className="mt-3 flex items-center justify-between gap-3">
        <button type="button" onClick={undo} disabled={!history.length || saving} className="rounded-lg border border-gray-700 px-3 py-2 text-sm font-bold text-gray-300 disabled:opacity-40">UNDO</button>
        <div className={`text-sm font-black ${message === "Bust" ? "text-red-300" : "text-gray-400"}`}>{saving ? "Gemmer..." : message}</div>
      </div>
    </div>
  );
}
